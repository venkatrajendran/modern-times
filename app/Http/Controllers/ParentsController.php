...
class ParentsController extends Controller {
	...
	public function export(){
		if($this->data['users']->role != "admin") exit;
		$data = array(1 => array ( 'Full Name','User Name','E-mail','adhar','Gender','Address','Phone No','Mobile No','birthday','Profession','password','ParentOf'));
		...
		foreach ($student as $value) {
			...
			$data[] = array ($value->fullName,$value->username,$value->email,$value->adhar,$value->gender,$value->address,$value->phoneNo,$value->mobileNo,date('d/m/Y',$value->birthday),$value->parentProfession,"",$value->parentOf);
		}
	...
	public function exportpdf(){
		if($this->data['users']->role != "admin") exit;
		$header = array ('Full Name','User Name','E-mail','adhar','Gender','Address','Phone No','Mobile No');
		$data = array();
		$student = \User::where('role','parent')->get();
		foreach ($student as $value) {
			$data[] = array ($value->fullName,$value->username ,$value->email,$value->adhar,$value->gender,$value->address,$value->phoneNo,$value->mobileNo);
		}
			...
			if ( $_FILES['excelcsv']['tmp_name'] )
			{
				...
				foreach ($readExcel as $row)
				{
                    ...
                    $importItem = array(
                        ...
                        'parentProfession' => (isset($row['profession']) && !empty($row['profession']))?$row['profession']:'',
                        'parentOf' => (isset($row['parentof']) && !empty($row['parentof']))?$row['parentof']:'',
	...
	public function searchStudents($student){
		...
		foreach ($students as $student) {
			$retArray[$student->id] = array("id"=>$student->id,"adhar"=>$student->adhar,"name"=>$student->fullName,"email"=>$student->email);
		}
