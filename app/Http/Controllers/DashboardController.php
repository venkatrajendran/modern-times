...
class DashboardController extends Controller {
	...
	public function dashboardData(){
		...
		if($this->data['users']->role == "student"){
			...
			$invoicesCount = \DB::table('payments')->where('paymentStudent',$this->data['users']->id)->where('paymentStatus','=','1')->selectRaw('SUM(paymentAmount) as paymentAmount')->get();
			$invoicesDueCount = \DB::table('payments')
			->where('paymentStudent',$this->data['users']->id)
			/*->where('dueDate','<',time())*/
			->where('paymentStatus','!=','1')->selectRaw('SUM(paymentAmount) as paymentAmount')->get();
			...
			$invoicesCount = \DB::table('payments')->whereIn('paymentStudent',$studentId)->where('paymentStatus','=','1')->selectRaw('SUM(paymentAmount) as paymentAmount')->get();
			$invoicesDueCount = \DB::table('payments')->whereIn('paymentStudent',$studentId)->
			/*where('dueDate','<',time())->*/
			where('paymentStatus','!=','1')->selectRaw('SUM(paymentAmount) as paymentAmount')->get();
			...
			$invoicesCount = \DB::table('payments')->where('paymentStatus','=','1')->selectRaw('SUM(paymentAmount) as paymentAmount')->get();
			$invoicesDueCount = \DB::table('payments')->
			/*where('dueDate','<',time())->*/
			where('paymentStatus','!=','1')->selectRaw('SUM(paymentAmount) as paymentAmount')->get();
            if(empty($invoicesCount[0]->paymentAmount))
            {
            $toReturn['stats']['invoices'] = 0;   
            }
            else
		{
		    $toReturn['stats']['invoices'] = $invoicesCount[0]->paymentAmount;
		}
		if(empty($invoicesDueCount[0]->paymentAmount))
			{
			    $toReturn['stats']['dueInvoices'] = 0;
			}
			else
			{
			$toReturn['stats']['dueInvoices'] = $invoicesDueCount[0]->paymentAmount;    
