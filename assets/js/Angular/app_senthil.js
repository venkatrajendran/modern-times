...
schoex.controller('invoicesController', function(dataFactory,$scope,$sce,$rootScope,$route) {
    ...
    $scope.linkStudent = function(){
        ...
        if(datemask.length <= 0){
            alert('Please selete the date first');
            return;
        }
     ...
     $scope.linkStudentButton = function(){
        var searchAbout = $('#searchLink').val();
        var datemask = $('#datemask').val();
        var datemask = datemask.replace("/", "-");
        var datemask = datemask.replace("/", "-");
        if(searchAbout.length < 3){
            alert($rootScope.phrase.minCharLength3);
            return;
        }
        dataFactory.httpRequest('index.php/invoices/searchUsers/'+searchAbout+'/'+datemask).then(function(data) {
            $scope.searchResults = data;
        });
    }
    ...
    $scope.linkStudentFinish = function(student){
        if(!$scope.form.paymentStudent){
		 ...
		 if(!$scope.form.paymentStudentNew){
            $scope.form.paymentStudentNew = [];
        }
          ...
          if(typeof($scope.form.newpay) == "undefined"){
            $scope.form.newpay = [];
        }
        //if(!$scope.form.paymentStudent){
            $scope.form.pendingPaymentRows = [];
        //}
		
		
        console.log($scope.form.paymentStudent);
		console.log(student);
		for(var i = 0;i<student.pending_payment.length;i++)
        {
            console.log(student.pending_payment[i].id);
            //$scope.form.paymentStudentNew.push({'amount':student.fee[i].feeAmount,'title':student.fee[i].feeTitle});
            $scope.form.pendingPaymentRows.push({'id':student.pending_payment[i].id,'number':student.pending_payment[i].number,'feeamount':student.pending_payment[i].feeAmount,'paidamount':student.pending_payment[i].paid});
        }
		for(var i = 0;i<student.fee.length;i++)
		{
			console.log(student.fee[i].feeAmount);
		$scope.form.paymentStudentNew.push({'amount':student.fee[i].feeAmount,'title':student.fee[i].feeTitle});	
		$scope.form.paymentRows.push({'title':student.fee[i].feeTitle,'amount':student.fee[i].feeAmount,'pre':1,'id':student.fee[i].feeId});
		}
        $scope.form.paymentStudent.push({'id':student.id,'name':student.name});
        if(student.total != '')
        {
        $scope.form.newpay.push({'total':student.total,'paid':student.paid,'r_amount':student.r_amount});
        }
    ...
    $scope.removeStudent = function(index){
        ...
        if (confirmRemove == true) {
            ...
            $scope.form.paymentStudentNew.splice(index);
            $scope.form.paymentRows.splice(index);
            $scope.recalcTotalAmount();
        }
    }
    ...
    $scope.saveAdd = function(){
        ...
        dataFactory.httpRequest('index.php/invoices','POST',{},$scope.form).then(function(data) {
            ...
            if(data.status == "success"){
                ...
                //$location.url('http://localhost/dev/custom_print.php');
                window.open('custom_print.php', '_blank'); // in new tab
               // window.location = 'custom_print.php';
               //$scope.reload();
               //$route.reload();
                $scope.changeView('list');
    ...
    $scope.recalcTotalAmount = function(){
        ...
        /*
		angular.forEach($scope.form.paymentStudentNew, function(value, key) {
            $scope.form.paymentAmount = parseInt($scope.form.paymentAmount) + parseInt(value.amount);
        });
        */
